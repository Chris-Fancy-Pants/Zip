...
public class DeadzoneCamera : MonoBehaviour 
{
	...
	protected bool followingIndicator = false;
	bool endGame = false;
	public Vector3 endGamePos = Vector3.zero;
...
//			transform.position = Vector3.SmoothDamp (current, smoothPos, ref _currentVelocity, 0.1f);
			transform.position = new Vector3(target.transform.position.x + offset.x, target.transform.position.y + offset.y, -20);
		} else {
	...
	public void EndGame(Vector3 endPos) {
